...
def read_2d_data(f_input, col_idx=1):
    ...
    Parameters
    ----------
    f_input (str): The filename of the input file. 
    col_idx (int): The index (starting from 0) of the column to be read as the 
                   dependent variable. 

    Returns
    -------
    x_data (np.array): The data of independent variable read from the input file. 
    y_data (np.array): The data of dependent variable read from the input file. 
    """
    try:
...
def scale_data(data, conversion=None, factor=None, T=None):
    ...
    Parameters
    ----------
    data  (np.array): The input data to be scaled. 
    conversion (str): The string describing what units the conversion should 
                      be carried out between. Available conversions include "ps to ns",
                      "kT to kJ/mol", "kT to kcal/mol", "kJ/mol to kcal/mol", "degree to radian",
                      and their respective conversion in the opposite direction. Note that 
                      the list of available conversions does not mean to be comprehensive since
                      the user could just specify the scaling factor. 
    factor   (float): The scaling factor for data scaling. 
    T        (float): The temperature to be considered to convert 
                      energy units to kT or vice versa.
    ...
    conversion_dict = {
        'ns to ps': 1000, 'ps to ns': 1 / 1000,
        'kT to kJ/mol': c1, 'kJ/mol to kT': 1 / c1,
        'kT to kcal/mol': c1 * c3, 'kcal/mol to kT': 1 / (c1 * c3),
        'kJ/mol to kcal/mol': c3, 'kcal/mol to kJ/mol': 1 / c3,
        'degree to radian': c2, 'radian to degree': 1 / c2
    }
    ...
    if conversion is not None:
        if conversion in conversion_dict:
            ...
            raise ParameterError('The specified conversion is not available. \
...
def slice_data(data, truncate=None, retain=None):
    ...
    Parameters
    ----------
    data  (np.array): The input data to be sliced.
    truncate (float): The fraction of data to be truncated from the beginning.
    retain   (float): The fraction of data to be retained from the beginning.
...
def analyze_data(x, y, x_label, y_label, outfile):
    ...
    Parameters
    ----------
    x  (np.array): The data of variable x.
    y  (np.array): The data of variable y.
    x_label (str): The label of the x-axis.
    y_label (str): The lable of the y-axis.
    """
    L = Logging(outfile)
    ...
    L.logger(f'The average of {y_var}: {y_avg:.3f} (RMSF: {RMSF:.3f}, max: {max(y):.3f}, min: {min(y):.3f})')
